...
class FileController extends Controller
{
    ...
    public function getFile($filename)
    {
        ...
        return response()->download(public_path('file.png'), 'User Image');
    }

    /**
     * Fetch files by Type or Id
     * @param string $type File type
     * @param null $id File Id
    ...
    public function index(string $type, $id = null)
    {
        ...
        if (!is_null($id)) {
            ...
            $records_per_page = ($type == 'video') ? 1 : 15;

            $files = $model::where('type', $type)
                ->where('task_id', Auth::id())
     ...
     * @return JsonResponse     True if success, otherwise - false
    ...
    {
        $file = File::where('id', $id)->where('task_id', Auth::id())->first();
     ...
     * @return string   Type
     */
    private function getType(string $ext)
    {
    ...
    {
        return Auth::Task()->name . '_' . Auth::id();
    }
}
